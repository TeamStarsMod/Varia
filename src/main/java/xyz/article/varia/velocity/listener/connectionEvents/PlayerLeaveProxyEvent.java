...
public class PlayerLeaveProxyEvent {
    ...
    public PlayerLeaveProxyEvent(ProxyServer server, File config, Logger logger) {
        this.server = server;
        this.config = config;
        this.logger = logger;
    }
    ...
    public void onPlayerLeaveProxyEvent(DisconnectEvent event) {
        ...
        if ((boolean) configMap.get("PlayerLeaveProxyPrompt")) {
            String message = (String) configMap.get("PlayerLeaveProxyPromptContent");
            if (message != null) {
